...
type
  ...
  TWorkspaceProjectMainForm = class(TForm)
  ...
  public
    ...
    property PathToJavaTemplates: string read FPathToJavaTemplates
      write FPathToJavaTemplates;
    property PathToSmartDesigner: string read FPathToSmartDesigner
      write FPathToSmartDesigner;
    property AndroidProjectName: string read FAndroidProjectName
      write FAndroidProjectName;
    ...
    {eclipse or ant}
    property PackagePrefaceName: string read FPackagePrefaceName
      write FPackagePrefaceName;
    ...
    //-1: gdx 0: GUI project   1: NoGui project
...
var
  WorkspaceProjectMainForm: TWorkspaceProjectMainForm;
...
procedure TWorkspaceProjectMainForm.ListBoxNdkPlatformChange(Sender: TObject);
var
  api: string;
  intNdkApi: integer;
begin
  ...
  begin
    ...
    FNdkApi := api; //'android-'+
    StatusBarInfo.Panels.Items[0].Text :=
      '[NDK-' + IntToStr(FNDKVersion) + ' Api ' + FNdkApi + ']';
    ...
    if intNdkApi > 22 then
      ShowMessage('Warning: for compatibility with old devices [4.x, 5.x]' +
        sLIneBreak + 'is strongly recommended NDK API <= 22!');
...
begin
  ...
  begin
    ...
    if intApi < 29 then
    begin
      ShowMessage('AppCompat theme need Target Api >= 29' + sLineBreak +
...
begin
  ...
  begin
    ShowMessage('WARNING: "laz4Android 1.8.0/2.0.0" [out-of-box]' +
      sLineBreak + 'don''t support "ARMV7a + VFPv3"' + sLineBreak +
  ...
  begin
    ShowMessage('WARNING: "laz4Android 1.8.0/2.0.0" [out-of-box]' +
      sLineBreak + 'don''t support "aarch64"' + sLineBreak + sLineBreak +
      'Hint: Select "ARMv7a + Soft"');
  end;

  if (FInstructionSetIndex = 6) and (IsLaz4Android) then
  begin
    ShowMessage('WARNING: "laz4Android 1.8.0/2.0.0" [out-of-box]' +
      sLineBreak + 'don''t support "x86_64"' + sLineBreak + sLineBreak +
      'Hint: Select "x86"');
  end;

  if FInstructionSetIndex = 5 then
  begin
    minNdkApi := StrToInt(ListBoxNdkPlatform.Items.Strings[
      ListBoxNdkPlatform.ItemIndex]);
    if minNdkApi < 21 then
      ShowMessage('Warning: "aarch64" need NDK Api >= 21 ...');
  end;
...
function TWorkspaceProjectMainForm.GetFullJavaSrcPath(fullProjectName: string): string;
var
  strList: TStringList;
  Count: integer;
  path: string;
...
begin
  ...
  if not IsAllCharNumber(PChar(FMinApi)) then
    FMinApi := '14';
  if not IsAllCharNumber(PChar(FTargetApi)) then
    FTargetApi := '29';

  if StrToInt(FMinApi) > apiTarg then
    FMinApi := IntToStr(apiTarg);
  ...
  begin
    ...
    FSmallProjName := StringReplace(ComboSelectProjectName.Text, ' ',
      '', [rfReplaceAll]);
    ...
    if EditPackagePrefaceName.Text = '' then
      EditPackagePrefaceName.Text := 'org.lamw';
  ...
  begin
    ...
    FSmallProjName := aList.Strings[aList.Count - 1];
    ; //ex. "AppTest1"
    FPackagePrefaceName := '';
    aList.Free;
    if FModuleType > 0 then  //NoGUI
  ...
  begin
    ...
    CopyFile(FPathToJavaTemplates + DirectorySeparator + 'values' +
      DirectorySeparator + 'colors.xml',
      FAndroidProjectName + DirectorySeparator + 'res' +
      DirectorySeparator + 'values' + DirectorySeparator + 'colors.xml');
    ...
    begin
      ...
      tempStr := ComboBoxTheme.Text;
      // AppCompat.Light.DarkActionBar  or AppCompat.Light.DarkActionBar
      CopyFile(FPathToJavaTemplates + DirectorySeparator + 'values' +
        DirectorySeparator + tempStr + '.xml',
        FAndroidProjectName + DirectorySeparator + 'res' +
  ...
  begin
    ...
    begin
      if MessageDlg('Projec/Directory already Exists!', 'Re-Create "' +
        FAndroidProjectName + '" ?', mtConfirmation, [mbYes, mbNo], 0) = mrNo then
      begin
        ModalResult := mrCancel;
      end;
    end
    ...
    begin
      ...
      begin
        CreateDir(FAndroidProjectName + DirectorySeparator + 'obj' +
          DirectorySeparator + LowerCase(FJavaClassName));
      end;
...
function TWorkspaceProjectMainForm.DoNewPathToJavaTemplate(): string;
begin
...
procedure TWorkspaceProjectMainForm.WriteIniString(Key, Value: string);
var
  FIniFile: TIniFile;
begin
...
begin
  ...
  if not IsAllCharNumber(PChar(strTApi)) then
    ...
    tApi := StrToInt(strTApi);

  strMApi := ListBoxMinSDK.Items.Strings[ListBoxMinSDK.ItemIndex];
  if not IsAllCharNumber(PChar(strMApi)) then
    mApi := 14
  else
    mApi := StrToInt(strMApi);
...
function TWorkspaceProjectMainForm.GetNDKVersion(ndkRelease: string): integer;
var
...
function TWorkspaceProjectMainForm.TryGetNDKRelease(pathNDK: string): string;
var
  list: TStringList;
...
procedure TWorkspaceProjectMainForm.LoadPathsSettings(const fileName: string);
var
    ...
    with TIniFile.Create(fileName) do
      try
        ...
        begin
          ...
          frm.LabelPathTo.Caption :=
            'WARNING! Path to Java JDK: [ex. C:\Program Files (x86)\Java\jdk1.7.0_21]';
        ...
        begin
          frm := TFormPathMissing.Create(nil);
          frm.LabelPathTo.Caption :=
            'WARNING! Path to Android NDK:  [ex. C:\lamw\ndk10e]';
          if frm.ShowModal = mrOk then
          begin
        ...
        case FNDKIndex of
          ...
          6: FNDK := '>21';
          //[rapid solution] deprecated "platforms" directories have been removed....
        end;
        ...
        begin
          ...
          //0.8.3 new path!  ...\android_wizard\smartdesigner\java
        end;

        FPathToSmartDesigner := ReadString('NewProject', 'PathToSmartDesigner', '');
        if FPathToSmartDesigner = '' then
        begin
          Self.DoPathToSmartDesigner();
          //0.8.3 new path!  ...\android_wizard\smartdesigner
        end;
        ...
        begin
          tempList.LoadFromFile(FPathToJavaTemplates + DirectorySeparator +
            'Controls.java');
          ...
          begin
            ...
            if FileExists(FPathToJavaTemplates + DirectorySeparator +
              'gdx' + DirectorySeparator + 'jGdxForm.native') then
            begin
              gdxList.LoadFromFile(FPathToJavaTemplates + DirectorySeparator +
          ...
          tempList.SaveToFile(FPathToJavaTemplates + DirectorySeparator +
            'Controls.events');
          //old "ControlsEvents.txt"
          nativeMethodList.SaveToFile(FPathToJavaTemplates +
...
procedure TWorkspaceProjectMainForm.FormActivate(Sender: TObject);
var
...
begin
  ...
  StatusBarInfo.Panels.Items[2].Text :=
    '[Target] ' + GetCodeNameByApi(ListBoxTargetAPI.Items[ListBoxTargetAPI.ItemIndex]);
  ...
  try
    FindAllDirectories(listDirectories, GetPathToSmartDesigner() +
      PathDelim + 'AppTemplates', False);
    Count := listDirectories.Count;
    for i := 0 to Count - 1 do
    begin
      p := LastDelimiter(PathDelim, listDirectories.Strings[i]);
      ComboBoxTheme.Items.Add(Copy(listDirectories.Strings[i], p +
        1, Length(listDirectories.Strings[i])));
    end;
  finally
...
begin
  ...
  begin
    ...
    if ComboBoxThemeColor.ItemIndex = 0 then
  ...
  begin
    ...
    begin
      ShowMessage('Warning/Recomendation:' + sLineBreak + sLineBreak +
        '[LAMW 0.8.6.1] "AppCompat" [material] theme need:' + sLineBreak +
        ' 1. Java JDK 1.8' + sLineBreak +
        ...
        ' Hint: "Ctrl + C" to copy this content to Clipboard!');
    ...
    if ListBoxMinSDK.ItemIndex < 1 then
      ListBoxMinSDK.ItemIndex := 1;   //Api 14
...
procedure TWorkspaceProjectMainForm.CheckBoxSupportChange(Sender: TObject);
var
  apiMin: integer;
begin
...
procedure TWorkspaceProjectMainForm.cbBuildSystemCloseUp(Sender: TObject);
var
  s: string;
begin
...
begin
  ShowMessage('Warning/Recomendation:' + sLineBreak + sLineBreak +
    '[LAMW 0.8.6.1] "AppCompat" [material] theme need:' + sLineBreak +
    ' 1. Java JDK 1.8' + sLineBreak +
    ' 2. Gradle 6.6.1 [https://gradle.org/next-steps/?version=6.6.1&format=bin] or up' +
    sLineBreak + ' 3. Android SDK "plataforms" 29 + "build-tools" 29.0.3' +
    sLineBreak + ' 4. Android SDK/Extra  "Support Repository"' +
    sLineBreak + ' 5. Android SDK/Extra  "Support Library"' +
    sLineBreak + ' 6. Android SDK/Extra  "Google Repository"' +
...
procedure TWorkspaceProjectMainForm.SpeedButtonSDKPlusClick(Sender: TObject);
var
  lisDir: TStringList;
...
procedure TWorkspaceProjectMainForm.LoadSettings(const pFilename: string);
var
  auxInstSet: string;
  tagVersion: integer;
  i: integer;
...
begin
  with TInifile.Create(pFilename) do
    try
      ...
      if FInstructionSetIndex >= 0 then
        WriteString('NewProject', 'InstructionSet',
          IntToStr(Self.FInstructionSetIndex));
...
begin
  ...
  begin
    ...
    if Count >= 2 then
    begin
      if auxStr[1] = delimiter then
        auxStr[1] := newchar;
      if auxStr[Count] = delimiter then
        auxStr[Count] := newchar;
    end;
    Result := Trim(auxStr);
  end;
end;
